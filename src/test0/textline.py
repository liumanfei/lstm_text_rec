...
def plot_char2(char):#	x=char[0],y=char[1]
	x=char[0]
	y=char[1]
	x_list=[]
	y_list=[]
...
def get_sparse_char(char, TdistConst= 8,TcosConst = 0.99):#return sparse_char_x,sparse_char_y,chars_point_num,	chars_left_bound, chars_right_bound
	...
	for i in range(0,int(len(char)/2)):
		if char[2*i]==65535 :
			...
			if(T_dist_m>TdistConst and T_cos<TcosConst):
				...
				char_point_num+=1
				stroke_point_num+=1
	# print(len(sparse_char_x),len(sparse_char_y),len(chars_point_num), len(chars_left_bound) ,len (chars_right_bound))
	assert(len(sparse_char_x)==len(sparse_char_y))
...
def get_label_str(label_int):
	...
	for i in range(0,len(label_int)):
		label_str.append(char_str[label_int[i]-1])
	return label_str
def sample_generator(char,label,time_dense_size=160):#	x=char[0], y=char[1],  return sample_x_list,sample_y_list,sample_label_list
	#char need a structure of '[0]sparse_char_x,[1]sparse_char_y,[2]chars_point_num,[3]chars_left_bound, [4]chars_right_bound'
	...
	while (len(sample_x_list)+chars_point_num[char_index]) <= time_dense_size :
		# points_catch += chars_point_num[char_index]
		left_index = sum(chars_point_num[:(char_index)])
		right_index = sum(chars_point_num[:char_index+1])-1
...
def diff(x_list,y_list,time_dense_size=160): #return [[diff_x, diff_y, pen_state]*len(xlist)]
	channel=3
	diff_data = np.zeros([time_dense_size,channel])#up:1,down:0
	skip_count = 0
...
def read_dataset(data_dir,label_dir):#,data_size,time_dense_size) return data_list(每一个元素list里面包含x,y,65535),label_int_list
	...
	with open(label_dir,'rt') as f:
		...
		for line in lines:
			...
			label_int_list.append(elements[1:])
			data_list.append(data_int[point_num_sum*2:(point_num_sum+elements[0])*2])
			if elements[0]>2:
				point_num_sum += elements[0]
			else:
				print(label_dir,elements)

	# sparse = get_sparse_char(data_list[0])

	# sample = sample_generator(sparse,label_int_list[0])

	# diff_input = diff(sample[0],sample[1])

	# print_label(sample[2])
	# print(sample[0][:20])
	# print(sample[1][:20])
	# print(diff_input[0][:20])
	# print(diff_input[1][:20])
	# print(diff_input[2][:20])
	# plot_char2(sample)
	# input('wait')
...
class batch_generator(Callback): #dataset:[0]xy data with 65535,[1]label_int
	def __init__(self, dataset, batch_size = 32, label_classes = len(char_str)+1,absolute_max_label_len = 20, channel =3 ,time_dense_size = 160):

		textline_num = len(dataset[0])
		if  textline_num% batch_size!=0:
			...
			for i in range(0,len(random_textline_index)):
				dataset[0].append(dataset[0][random_textline_index[i]])
				dataset[1].append(dataset[1][random_textline_index[i]])

		self.dataset_size = len(dataset[0])
	...
	def get_batch(self,  cur_sample_index, train=True):
		...
		X_data = np.ones([self.batch_size, self.time_dense_size, self.channel])
		...
		for i in range(0, self.batch_size):
			...
			if train and i > self.batch_size:
				...
				# X_data[i] =self.data_textline_list[cur_sample_index+i]
				sparse_text = get_sparse_char(self.data_textline_list[cur_sample_index+i])

				sample_x,sample_y,sample_label = sample_generator(sparse_text,self.label_int_list[cur_sample_index+i],self.time_dense_size)

				diff_data = diff(sample_x,sample_y)

				X_data[i,:len(diff_data)] = diff_data
	...
	def next_train(self):
		while 1:
			ret = self.get_batch(self.cur_sample_index, train=True)
			self.cur_sample_index += self.batch_size
			if self.cur_sample_index >= self.dataset_size:
				self.cur_sample_index = self.cur_sample_index % self.dataset_size
			yield ret
	def next_val(self):
		while 1:
...
def train(start_epoch, stop_epoch):
	...
	train_data = batch_generator(dataset=data_trainset, batch_size = minibatch_size, label_classes = label_class_num, 
					absolute_max_label_len = absolute_max_label_length, channel =channel, time_dense_size = time_dense_size)
	test_data = batch_generator(dataset=data_testset, batch_size = minibatch_size, label_classes = label_class_num, 
					absolute_max_label_len = absolute_max_label_length, channel =channel, time_dense_size = time_dense_size)
	assert(K.image_data_format() != 'channels_first')
	...
	train_set_size_full_batch = train_data.get_dataset_size()
	validation_set_size_full_batch = test_data.get_dataset_size()
	...
	model.fit_generator(generator=train_data.next_train(), steps_per_epoch=2,#1391,int(train_data.get_dataset_size()/minibatch_size),
						epochs=stop_epoch, validation_data=test_data.next_val(), 
						validation_steps=int(test_data.get_dataset_size()/minibatch_size/100+1),#shuffle=True,
