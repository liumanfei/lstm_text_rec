...
def get_label_str(label_int):
	...
	for i in range(0,len(label_int)):
		label_str.append(char_str[label_int[i]-1])
	return label_str
def plot_char(char,figure_name = 'Original'):
	x_list=[]
	y_list=[]
	x_list_array = []
	y_list_array = []
...
def plot_char2(char,figure_name = 'get_sparse_line'):#	x=char[0],y=char[1]
	x=char[0]
	y=char[1]
	x_list=[]
	y_list=[]
	...
	plt.title(figure_name)
	plt.show()
def plot_char3(char_label,figure_name = 'get_sparse_diff_line'):
	line = char_label[0]
	...
	plt.title(figure_name)
	plt.show()		
def plot_char4(diff_data,figure_name = 'diff'):
	x_list = []
	y_list = []
	x = [diff_data[0][0]]
	y = [diff_data[0][1]]
...
def move_x(x_list,x_move):
	...
	return x_list
def sample_generator0(char,label,time_dense_size=160):# return sample_x_list,sample_y_list,sample_label_list
...
def sample_generator(data_label,absolute_max_label_len,time_dense_size=160,channel = 3):#	[[x,y,state],label],  return [[x,y,state],label]
	...
	while point_count+chars_point_num[char_index] <= time_dense_size and len(sample_label)<absolute_max_label_len :
		...
		if char_index<chars_num-1:
			...
			char_index = 0
	sample_data[point_count-1] = [0,0,1]
...
class batch_generator(Callback): #dataset:[0]xy data with 65535,[1]label_int
	def __init__(self, data_label_list, batch_size = 32, label_classes = len(char_str)+1,absolute_max_label_len = 20, channel =3 ,time_dense_size = 160):
...
def train(start_epoch, stop_epoch):
	...
	model.fit_generator(generator=train_data.next_train(), steps_per_epoch=1391,#1391,int(train_data.get_dataset_size()/minibatch_size),
